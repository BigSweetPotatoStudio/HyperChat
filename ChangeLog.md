...
* Fixed the issue of failing to input prompts that support mcp
...
# 1.5.4
* Fixed the issue of adding large model test failures in web access.

# 1.5.3

* Supports Claude official API.
* Optimized the built-in terminal, allowing user input
* Supports Agent task calls failing, using a backup large model.
...
* Tool call cancellation optimization, inform the large model that the user has canceled the operation
...
![Supports message branching cloning](https://github.com/user-attachments/assets/498d4e03-0555-4b9b-9838-ec46602fb501)
...
![Supports global control enable/disable for Task running](https://github.com/user-attachments/assets/7537941a-1e2d-41a8-abb6-2569fe040067)
...
![Tool call cancellation optimization, informing the large model that the user has canceled the operation](https://github.com/user-attachments/assets/8b1186b3-929f-4c86-95ce-50dbf2216f01)



# 1.4.17
* Fixed the issue of inability to copy on MacOS, shortcuts becoming ineffective

# 1.4.16

* Supports displaying MCP Server version and name.
* Added detailed errors when the model reports an error.
* Added detailed errors when the MCP reports an error.
...
# 1.4.3
* Rewrote markdown rendering, optimized `Artifacts`, supports opening and downloading from the browser, increased support for `Mermaid` preview,
...
* Fixed the issue in the previous version where the default conversation setting for mcp became ineffective.
* Fixed the issue where Agents could not modify system prompts when not chatting.
* Fixed the error prompt for no LLM when opened for the first time


# 1.4.1

* Fixed the bug where Gemini tool calls did not support multiple tools
* Supports selecting partial Tools of MCP, saving more tokens
...
* Added built-in mcp tools command line
...
* Can confirm when calling tools, and modify the parameters for large model calls
* Can click on tools to call them for testing.
* The knowledge base is intended to be redeveloped; it is currently not recommended for use, and users can use OpenAI's embedding model instead of maintaining a local embedding model
...
* Fixed MacOS image uploads
* Added a quick copy button, supports copy message and code buttons
...
* Supports two installation methods: plugin market + MCP native installation, plugin market quick installation and configuration, welcome to submit plugins, [Github](https://github.com/BigSweetPotatoStudio/HyperChatMCP)
* WebDav synchronization logic optimization, locally backs up before synchronizing, syncing every 5 minutes
* LLM added testing function, testing whether it supports image input + tool calls (consuming a little token)
...
* WebDav synchronization, currently does not support MCP configuration synchronization, as inconsistencies in operating systems may cause issues.
...
* For APIs that do not support counting token consumption for large models, such as Qianwen, supports fuzzy counting by word count
* Input box supports drag and drop file for quick input of file paths
* My LLM Models list supports drag-and-drop sorting
* Supports quickly configuring Ollama, Qwen's API models

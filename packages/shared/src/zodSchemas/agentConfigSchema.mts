...
export const BaseAIConfigSchema = z.object({
  ...
  maxTokens: z.number()
    .int()
    .min(1000, "Max tokens must be >= 100")
  ...
  maxContextTokens: z.number()
    .int()
    .min(8000, "Max context tokens must be >= 1000")
    .default(32000)
    .optional()
  ...
  compressionStrategy: z.literal("tokens")
    .default("tokens")
    .optional()
    .describe("Compression strategy: fixed to tokens")
});
...
export function createDefaultBaseAIConfig(prompt: string): BaseAIConfig {
  return BaseAIConfigSchema.parse({ prompt });
}

// 默认Agent配置工厂函数
export function createDefaultAgentConfig(name: string, prompt: string): AgentConfig {
  return AgentConfigSchema.parse({ name, prompt });
}
...
export function createSmartBaseAIConfig(
  ...
  if (modelKey) {
    ...
    config.compressionStrategy = "tokens";
  }

...
export function registerReadManyFilesTool(server: McpServer, workspacePath: string): void {
  server.tool(
    'read_many_files',
    ...
    async ({ absolute_paths, include_metadata, max_lines_per_file, skip_errors }) => {
      ...
      try {
        ...
        const normalizedPaths = absolute_paths.map(filePath => 
          validateAndNormalizePath(filePath, workspacePath)
        );
        
        // 并行读取所有文件
        const readPromises = normalizedPaths.map(filePath => 
          readSingleFile(filePath, workspacePath, max_lines_per_file, include_metadata)
        );
